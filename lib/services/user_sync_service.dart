...
class UserSyncService {
  ...
  Future<List<Map<String, dynamic>>> getSynchronizedUsers() async {
    try {
      ...
      if (serverResponse['success'] == true && 
        ...
        print('📡 Server accounts retrieved: ${serverAccounts.length}');
  ...
  Future<Map<String, dynamic>> getSyncStatus() async {
    try {
      ...
      final nonAdminLocalUsers = localUsernames
          .where((username) => !_databaseHelper.isAdminUser(username))
          .toSet();
      ...
      return {
        ...
        'users_only_local': nonAdminLocalUsers.difference(serverExternalIds).toList(),
        'users_only_server': serverExternalIds.difference(nonAdminLocalUsers).toList(),
        'users_on_both': nonAdminLocalUsers.intersection(serverExternalIds).toList(),
        'admin_protected': localUsernames.where((u) => _databaseHelper.isAdminUser(u)).toList(),
      };
    } catch (e) {
      return {
        'error': e.toString(),
